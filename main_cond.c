...
//define and initialization of condition variable
...
//Flag representing state of a car being on the bridge
...
void* Przejazd(void* vargp)
{
    ...
    while(1)
    {
        ...
        pthread_mutex_lock(&lock);
        //if someone is driving over the bridge wait for signal
        while(jedzie){
            pthread_cond_wait(&condition,&lock);
        }
        //set variable that represents that someone is on the bridge
        jedzie = 1;
        ...
        //randomized sleeping time for each thread
        usleep(rn(1000,100000));
        //set variable that represents that someone drove off the bridge
        jedzie = 0;
        //send signal
